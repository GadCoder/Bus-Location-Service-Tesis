...
from app.repository.distance_from_user import calculate_distance_from_user


async def save_bus_location(bus_location: BusLocationCreate):
    bus_in_db = await BusLocation.find_one(
        BusLocation.bus.bus_identifier == bus_location.bus.bus_identifier
    )
    if not bus_in_db:
...
async def get_buses_from_route(
    company_id: int,
    route_id: int,
    latitude: float,
    longitude: float,
    delay_in_min: int,
    max_distance_in_km: int = 1,
):
